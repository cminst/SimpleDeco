...
from nemo.collections.llm import Qwen25Config7B, Qwen2Model, Qwen2Config
...
@dataclass
class AutoDecoQwen25Config7B(Qwen25Config7B):
...
class AutoDecoQwen2ModelNeMo(Qwen2Model):
    def __init__(
            self,
    ...
    def configure_model(self, vp_stage: Optional[int] = None) -> None:
        ...
        if parallel_state.is_pipeline_last_stage() and first_configure and not hasattr(self.module, "temp_head"):
            ...
            if vps is None or vp_stage is None or vp_stage == vps - 1:
                ...
                save_file(tensors=adapter.state_dict(), filename="/root/empty_qwen25_head.safetensors")
    ...
    def forward(
        ...
        # print("max token id: ", batch["tokens"].max())
        # print("min token id: ", batch["tokens"].min())
        #
        outputs = self.module(
            batch["tokens"],
            batch["position_ids"],
...
@io.model_exporter(AutoDecoQwen2ModelNeMo, "hf-peft")
class HFAutoDecoQwen2ModelExporter(io.ModelConnector[AutoDecoQwen2ModelNeMo, "AutoModelForCausalLM"]):
