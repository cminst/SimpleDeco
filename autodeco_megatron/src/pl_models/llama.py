...
from nemo.collections.llm import LlamaModel, Llama31Config8B
...
@dataclass
class AutoDecoLLama31Config8B(Llama31Config8B):
...
class AutoDecoLlamaModelNeMo(LlamaModel):
    def __init__(
            self,
            config: Annotated[Optional[AutoDecoLLama31Config8B], Optional[AutoDecoLLama31Config8B]] = None,
    ...
    def configure_model(self, vp_stage: Optional[int] = None) -> None:
        ...
        if parallel_state.is_pipeline_last_stage() and first_configure and not hasattr(self.module, "temp_head"):
            ...
            if vps is None or vp_stage is None or vp_stage == vps - 1:
                ...
                save_file(tensors=adapter.state_dict(), filename="/root/empty_llama_head.safetensors")
...
@io.model_exporter(AutoDecoLlamaModelNeMo, "hf-peft")
class HFAutoDecoLlamaModelExporter(io.ModelConnector[AutoDecoLlamaModelNeMo, "AutoModelForCausalLM"]):
