...
@dataclass
class AutoDecoDeepSeekV3Config(DeepSeekV3Config):
...
class AutoDecoDeepSeekV3ModelNeMo(DeepSeekModel):
    def __init__(
            self,
            config: Annotated[Optional[AutoDecoDeepSeekV3Config], Optional[AutoDecoDeepSeekV3Config]] = None,
    ...
    def configure_model(self, vp_stage: Optional[int] = None) -> None:
        ...
        if parallel_state.is_pipeline_last_stage() and first_configure and not hasattr(self.module, "temp_head"):
            ...
            if vps is None or vp_stage is None or vp_stage == vps - 1:
                ...
                save_file(tensors=adapter.state_dict(), filename="/root/empty_deepseek_head.safetensors")
...
@io.model_exporter(AutoDecoDeepSeekV3ModelNeMo, "hf-peft")
class HFAutoDecoDeepSeekV3ModelExporter(io.ModelConnector[AutoDecoDeepSeekV3ModelNeMo, "AutoModelForCausalLM"]):
