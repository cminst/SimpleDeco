...
# Auto-download required NLTK resources on first import.
# Data is stored in the user's ~/nltk_data directory and is NOT committed to
# this repo.  NLTK >= 3.9 replaced the old punkt pickle with punkt_tab; we
# download both so the code works on any installed version.
for _resource, _check_path in [
    ("punkt_tab", "tokenizers/punkt_tab"),
    ("punkt",     "tokenizers/punkt"),
]:
    try:
        nltk.data.find(_check_path)
    except LookupError:
...
def _get_sentence_tokenizer():
  try:
    # NLTK < 3.9: punkt stored as a pickle file.
    return nltk.data.load("nltk:tokenizers/punkt/english.pickle")
  except LookupError:
    # NLTK >= 3.9: punkt replaced by punkt_tab with a new API.
    return nltk.tokenize.PunktTokenizer("english")
